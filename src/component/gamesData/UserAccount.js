...
export default function UserAccount() {
    ...
    async function getUserBats(setUser_data, setJoinBat, setMyBat, setCombine, joinBat, myBat) {
...
//_________________________________________________________________________________________________
    ...
    return (
        <div>
            <button onClick={() => getUserBats(setUser_data, setJoinBat, setMyBat, setCombine, joinBat, myBat)}> Account </button>
            {showAccount ? <div className="" style={{ display: 'inline-block', color: "black" }}>
                <AccountPanel myBat={myBat} joinBat={joinBat} combine={combine} setShowAccount={setShowAccount} user_data={user_data}/> </div>
...
function AccountPanel({ myBat, joinBat, combine , setShowAccount , user_data }) {
        ...
        <div className="account-panel">
            ...
            <div>  <h3>open bats</h3>
                ...
                {/* {myBat.map((bat) => bat.match_ok === "false" ? <SingelBat_On userTeam={bat.chosen_team_id} userTeamName={bat.chosen_team_name} bat={bat} key={bat.id} /> : */}
                    {myBat.map((bat) => bat.match_ok === "false" ? <SingelBat_Open user_data={user_data} userTeam={bat.chosen_team_id} userTeamName={bat.chosen_team_name} bat={bat} key={bat.id} /> :
                        <div > </div>)}

                </div>
                <div style={{ color: "blue" }}>
                {joinBat.map((bat) => bat.match_ok === "false" ? <SingelBat_Open user_data={user_data} userTeam={bat.opp_team_id} userTeamName={bat.opp_team_name} bat={bat} key={bat.id} /> :
                ...
                <div>  <h3>accepted bats</h3>
                    <div style={{ color: "green" }}>
                        {myBat.map((bat) => bat.match_ok === "true" ? <SingelBat_Open user_data={user_data} userTeam={bat.chosen_team_id} userTeamName={bat.chosen_team_name} bat={bat} key={bat.id} /> :
                            <div > </div>)}

                    </div>
                    <div style={{ color: "green" }}>
                        {joinBat.map((bat) => bat.match_ok === "true" ? <SingelBat_Open user_data={user_data} userTeam={bat.opp_team_id} userTeamName={bat.opp_team_name} bat={bat} key={bat.id} /> :
...
function SingelBat_Open({user_data, userTeam, userTeamName, bat }) {
    ...
    // //_________________________________________ MATCH DATA________________________________________
    // console.log("match hour : ", match_hour)
    // console.log("match minutes : ", match_minute)
    // // console.log("match hour : " , the_data.getHours(the_data))

    // console.log("match year :", match_year)
    // console.log("match month : ", match_month)
    // console.log("match day in month", match_day_in_month)


    //_________________________________________ CURRENT DATA TIME________________________________________
    // console.log("current hour : ", date.getHours(date))
    // console.log("current minutes : ", date.getMinutes(date))
    // // console.log("match hour : " , the_data.getHours(the_data))

    // console.log("current year :", date.getFullYear(date))
    // console.log("current month : ", date.getMonth(date) + 1)
    // console.log("current day in month", date.getDate(date))
        ...
        <div className="">
            {bat_steel_good === "true" ? <>
                ...
                <div className="row">
                    ...
                    <div className="account-col col-3">
                        {bat.match_ok === "true" ? <div> </div> : <button className="btn" style={{ backgroundColor: "gray" }}>cancel</button>}  {/* {bat.id}*/}
